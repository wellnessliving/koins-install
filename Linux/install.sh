...
software="mc apache2 mysql-server php7.2 php7.2-bcmath php7.2-xml php7.2-curl php7.2-gd php7.2-mbstring php7.2-mysql php7.2-soap php7.2-tidy php7.2-zip php-apcu php-memcached memcached crudini libneon27-gnutls libserf-1-1 jq subversion"
...
help_message() {
  ...
  -lu, --linux-user         Linux user                     required
...
# Parsing arguments
while getopts "lu:b:a:s:k:p:d:c:n:l:m:g:x:w:t:fh" Option; do
  case ${Option} in
  	lu) linux_user=$OPTARG ;;      # Linux user
...
set_default_value 'linux_user' 'petrich'
...
set_default_value 'workspace' '/var/www/wellness'
...
printf "Checking set argument --linux-user: "
if [[ ! -n "${linux_user}" ]]; then
  check_result 1 "Linux user not set. Try 'bash $0 --help' for more information."
fi
echo "[OK]"
...
if [[ -d "${unix_workspace:0:8}" ]]; then #workspace=/var/www/wellness   ${workspace:0:8}=> /var/www
...
crudini --set /etc/php/7.2/apache2/php.ini PHP max_execution_time "600"
crudini --set /etc/php/7.2/cli/php.ini PHP max_execution_time "600"
...
chmod -R 777 ${unix_workspace}
chown -R www-data:www-data ${unix_workspace}
cp -rf /root/.ssh/libs.key /home/${linux_user}/.ssh/libs.key
cp -rf /root/.ssh/libs.key /home/${linux_user}/.ssh/libs.pub
chmod 600 /home/${linux_user}/.ssh/libs.key
chmod 600 /home/${linux_user}/.ssh/libs.pub
crudini --set /home/${linux_user}/.subversion/config tunnels libs "ssh svn@libs.svn.1024.info -p 35469 -i ~/.ssh/libs.pub"
echo "127.0.0.1 ${host_trunk}" >> /etc/hosts
echo "127.0.0.1 ${host_stable}" >> /etc/hosts
echo "127.0.0.1 ${host_production}" >> /etc/hosts
...
Key for repository 'libs' saved in ${unix_workspace}\\keys\\libs.key${NC}"
